...
from tensorflow.keras.layers import Conv3D, concatenate, Conv3DTranspose, Average, add
...
def unetpp(multi_modal, early_fusion, project_excite, inputs_bmode, inputs_pd, cascade=False):
    ...
    if mode == 'cascade':  # used in the competition
        for i in range(depth):
            conv4_0 = Conv3D(n_filters[4], 3, activation='relu', padding='same', dilation_rate=2**i)(conv4_0)
            dilated_layers.append(conv4_0)
        conv4_0 = add(dilated_layers)
    elif mode == 'parallel':  # Like "Atrous Spatial Pyramid Pooling"
        for i in range(depth):
            dilated_layers.append(
                Conv3D(n_filters[4], 3, activation='relu', padding='same', dilation_rate=2**i)(conv4_0)
            )
        conv4_0 = add(dilated_layers)
