from tensorflow.keras.layers import Conv3D, Dropout, Input, BatchNormalization, MaxPooling3D, concatenate, Multiply, Add, ReLU
...
"""Multi level dilated convolutional block"""
def mldc(input, n_filters, project_excite=False, k_size=3):
    i = input
    x = Conv3D(n_filters, k_size, padding='same')(input)
    x = BatchNormalization()(x)
    x = ReLU()(x)
    x = Conv3D(n_filters, k_size, padding='same')(x)
    x = BatchNormalization()(x)
    x = Conv3D(n_filters, k_size, dilation_rate=(3, 3, 3), padding='same')(x)
    x = BatchNormalization()(x)
    x = ReLU()(x)
    x = Conv3D(n_filters, k_size, dilation_rate=(3, 3, 3), padding='same')(x)
    x = BatchNormalization()(x)
    x = Conv3D(n_filters, k_size, dilation_rate=(5, 5, 5), padding='same')(x)
    x = BatchNormalization()(x)
    x = ReLU()(x)
    x = Conv3D(n_filters, k_size, dilation_rate=(5, 5, 5), padding='same')(x)
    x = BatchNormalization()(x)
    y = Conv3D(n_filters, kernel_size=1, padding='same')(i)
    y = BatchNormalization()(y)
    x = x + y
    x = ReLU()(x)


"""Multi level dilated skip connection"""
def mlrs(input, n_filters, project_excite=False, k_size=3):
    pass
...
def handle_input_fusion(multi_modal, input_shape, batch_size):
    ...
    print(f'handle_input_fusion: {input_bmode}')
    input_pd = None
    if multi_modal:
        input_pd = Input(shape=input_shape, batch_size=batch_size)
        print(f'handle_input_fusion: {input_pd}')
