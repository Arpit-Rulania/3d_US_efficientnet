...
def decode(serialized_example, modal_type):
    ...
    if modal_type == "multi_modal":
        ...
        #input_vol = tf.stack([input_vol] * 3, axis=-1)

    label = tf.io.decode_raw(features['label'], tf.uint8)
    label = tf.reshape(label, volume_shape)
    label = tf.expand_dims(label, axis=-1)
    label = tf.cast(label, tf.float32)
    #label = tf.stack([label, label, label], axis=-1)

    #print("ARPIT ARPIT ARPIT ARPIT ARPIT ARPIT ARPIT ARPIT ARPIT ARPIT")
    #print("Input shape:", input_vol.shape)
