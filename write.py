    ...
    while batch_count <= len(preprocessed_volumes):
        with tf.io.TFRecordWriter(get_record_path(dataset_type, batch_start, augment_data)) as writer:
            for j, volume_tuple in enumerate(preprocessed_volumes[batch_start:batch_count]):
                ...
                """
                bmode = tf.tile(tf.expand_dims(bmode, axis=-1), multiples=[1, 1, 1, 3])
                pd = tf.tile(tf.expand_dims(pd, axis=-1), multiples=[1, 1, 1, 3])
                label = tf.tile(tf.expand_dims(label, axis=-1), multiples=[1, 1, 1, 3])
                
                bmode = tf.repeat(tf.expand_dims(bmode, axis=-1), repeats=3, axis=-1)
                pd = tf.repeat(tf.expand_dims(pd, axis=-1), repeats=3, axis=-1)
                label = tf.repeat(tf.expand_dims(label, axis=-1), repeats=3, axis=-1)"""
